    ...
    public function setCredentials($mid = null, $key = null)
    {
        try {
            if ((ctype_digit((string)$mid)) && (strlen($key) == 24)) {
                ...
                throw new InvalidArgumentException('[maxiPago Class error] Invalid credentials.', 401);
            }
        } catch (Exception $e) {
            if (is_object(maxiPago_RequestBase::$logger)) {
    ...
    public function setEnvironment($param = null)
    {
        try {
            if (strtoupper($param) == 'TEST') {
                ...
                throw new BadMethodCallException('[maxiPago Class error] Invalid environment. ' . __METHOD__ . ' accepts either "TEST" or "LIVE"', 400);
            }
            if (is_object(maxiPago_RequestBase::$logger)) {
